...
internal class ApplicationDbSeeder
{
    ...
    public ApplicationDbSeeder(FSHTenantInfo currentTenant, RoleManager<ApplicationRole> roleManager, UserManager<ApplicationUser> userManager, CustomSeederRunner seederRunner, IJobService jobService, ILogger<ApplicationDbSeeder> logger)
    {
    ...
    private async Task SeedRolesAsync(ApplicationDbContext dbContext)
    {
        foreach (string roleName in FSHRoles.DefaultRoles)
        {
            ...
            else if (roleName == FSHRoles.Employer)
            {
                await AssignPermissionsToRoleAsync(dbContext, FSHPermissions.Employer, role);
            }
            else if (roleName == FSHRoles.Applicant)
            {
                await AssignPermissionsToRoleAsync(dbContext, FSHPermissions.Applicant, role);
            }
    ...
    private async Task SeedLookupDataAsync(ApplicationDbContext dbContext)
    {
        // Governorates
        if (!dbContext.Governorate.Any())
        {
            dbContext.Governorate.AddRange(new List<Governorate>
        {
            new Governorate("Cairo"),
            new Governorate("Giza"),
            new Governorate("Port Said"),
            new Governorate("Damietta"),
            new Governorate("Beheira"),
        });
        ...
        // Districts
        if (!dbContext.District.Any())
        {
            dbContext.District.AddRange(new List<District>
        {
            new District("Shobra", 1),
            new District("Kafr Ramadah", 2),
        });
            await dbContext.SaveChangesAsync();
        }
    }
